...
data TargetFlags = TargetFlags
  { targetVerbosity :: Flag Verbosity
  , targetDistDir :: Flag FilePath
  , targetListSources :: Flag Bool
  , targetNulSeparated :: Flag Bool
  , targetOutputPath :: Flag FilePath
  }

defaultTargetFlags :: TargetFlags
defaultTargetFlags =
  TargetFlags
    { targetVerbosity = toFlag normal
    , targetDistDir = mempty
    , targetListSources = toFlag False
    , targetNulSeparated = toFlag False
    , targetOutputPath = mempty
    }
...
targetOptions showOrParseArgs =
  [ optionVerbosity
      targetVerbosity
      (\v flags -> flags{targetVerbosity = v})
  , optionDistPref
      targetDistDir
      (\dd flags -> flags{targetDistDir = dd})
      showOrParseArgs
  , option
      ['l']
      ["list-only"]
      ...
      targetListSources
      (\v flags -> flags{targetListSources = v})
      trueArg
  , option
      []
      ["null-sep"]
      ...
      targetNulSeparated
      (\v flags -> flags{targetNulSeparated = v})
      trueArg
  , option
      ['o']
      ...
      targetOutputPath
      (\o flags -> flags{targetOutputPath = o})
...
targetAction (pf@ProjectFlags{..}, TargetFlags{..}) targetStrings globalFlags = do
  ...
  case reifyTargetSelectors localPkgs targetSelectors of
    ...
    Right pkgs
      ...
      | otherwise ->
          traverse_ (\pkg -> packageToTarget verbosity (distProjectRootDirectory distDirLayout) format (outputPath pkg) pkg) pkgs
  where
    verbosity = fromFlagOrDefault normal targetVerbosity
    listSources = fromFlagOrDefault False targetListSources
    nulSeparated = fromFlagOrDefault False targetNulSeparated
    mOutputPath = flagToMaybe targetOutputPath
        ...
        (defaultNixStyleFlags ())
          { configFlags =
              (configFlags $ defaultNixStyleFlags ())
                { configVerbosity = targetVerbosity
                , configDistPref = targetDistDir
                }
          , projectFlags = pf
          }
        mempty
...
packageToTarget :: Verbosity -> FilePath -> OutputFormat -> FilePath -> UnresolvedSourcePackage -> IO ()
packageToTarget verbosity projectRootDir format outputFile pkg = do
