...
module UnitTests.Distribution.Client.CmdBuildOptionsCommandUI (tests) where

import Distribution.Client.CmdBuild (buildCommand)
...
tests :: [TestTree]
tests =
  [ testCase "build CommandUI parser recognizes options advertised by CommandUI(ParseArgs)" $ do
      let probes = commandOptionProbes buildCommand
          failures =
            [ (probeDisplay probe, unlines errs)
            | probe <- probes
            , CommandErrors errs <- [commandParseArgs buildCommand False (probeArgv probe)]
...
isUnrecognizedError msg =
  let lower = map toLower msg
   in "invalid option" `isInfixOf` lower
        ...
        || "unrecognised" `isInfixOf` lower
        || "unknown option" `isInfixOf` lower
