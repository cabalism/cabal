...
main = do
  ...
  cabalTest' "parser-legacy" . recordMode RecordMarked $ runTest "legacy"
  cabalTest' "parser-default" . recordMode RecordMarked $ runTest "default"
  cabalTest' "parser-parsec" . recordMode RecordMarked $ runTest "parsec"
  cabalTest' "parser-fallback" . recordMode RecordMarked $ runTest "fallback"
  where
    log = recordHeader . pure

    runTest parser = do
      log "checking that we detect when the same config is imported via many different paths"
      yopping <- cabal' "v2-build" [ "--project-file=yops-0.project", "--project-file-parser=" ++ parser, "--dry-run" ]
      ...
      log "checking that we detect when the same config is imported via many different paths"
      wooping <- cabal' "v2-build" [ "--project-file=woops-0.project", "--project-file-parser=" ++ parser, "--dry-run" ]
